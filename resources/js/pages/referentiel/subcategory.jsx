...
export default function Category() {
    const { auth, sub_categories, categories } = usePage().props;
    ...
    async function handleGetSubCategory(id) {
        try {
            const response = await axios.get(`/sous-famille/show?id=${id}`, {
                headers: {
            ...
            modalEditControl();
            setSubCategory(response.data.sousCategory);
        } catch (error) {
    ...
    const handleEditSubCategory = (e) => {
        ...
        router.put(`/sous-famille/${subCategory.id}/edit/`, subCategory, {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                modalEditControl();
    ...
    const handleSelectAll = (event) => {
        if (event.target.checked) {
            const newSelected = sub_categories && sub_categories.map((product) => product.id);
            ...
            <div className="flex h-full flex-1 flex-col gap-4 rounded-xl p-4">
                <div className="border-sidebar-border/70 dark:border-sidebar-border relative min-h-[100vh] flex-1 overflow-hidden rounded-xl border md:min-h-min">
                        ...
                        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(0,0,0,0.6)]">
                            <div className="bg-[rgba(255,255,255,0.1)] bg-opacity-10 backdrop-blur-3xl rounded-lg w-full max-w-xl p-6 shadow-lg">
                                ...
                                <form onSubmit={handleEditSubCategory} className="space-y-4">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <label htmlFor="status">Status : </label>
                                        <ToggleSwitch initialStatus={subCategory.status} onToggle={handleStatusChangeEdit} />
                                    </div>
                    ...
                    {sub_categories && sub_categories.length > 0 ? (
                                    ...
                                    <tr className='text-xl'>
                                        <th className="text-left px-4 py-4">
                                            <input
                                                ...
                                                checked={selected.length === sub_categories.length && sub_categories.length > 0}
                                                className="form-checkbox text-blue-600"
                                            />
                                        </th>
                                        <th className="text-left px-4 py-4">_ID</th>
                                        <th className="text-left px-4 py-4">Nom Sous Famille</th>
                                    ...
                                    {filteredSubCategories.map((product) => {
                                        ...
                                        return (
                                            <tr key={product.id} className={isItemSelected ? 'bg-gray-900' : ''}>
                                                ...
                                                <td className="px-4 py-4">
                                                    {categories.find((cat) => cat.id === product.cat_id)?.intitule || '—'}
                                                </td>
                                                <td className="px-4 py-4">
                                                    {product?.status === 1 ? (
                        ...
                        // Content if there are no sub_categories
